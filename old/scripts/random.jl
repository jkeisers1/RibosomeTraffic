...
Plots.scatter!(
    ...
    markersize = 6,
    xlabel = "log(kp)",
    ylabel = "Time until first pause T0",
)
...
densities_1350 = calculate_state_densities_corrected_plateau4.(
    1350,
    kp_list,
    km,
    l_ribosome,
...
densities_100 = calculate_state_densities_corrected_plateau4.(
    100,
    kp_list,
    km,
    l_ribosome,
...
densities_1350 = calculate_state_densities_corrected_plateau.(
    1350,
    kp_list,
    km,
    l_ribosome,
...
densities_100 = calculate_state_densities_corrected_plateau.(
    100,
    kp_list,
    km,
    l_ribosome,
...
Plots.scatter!(
    ...
    Pp_100_num,
    xticks = -5:1:-1,
    label = "L:100, Unpaused Numerical",
    markersize = 6,
)
Plots.plot!(
    log10.(kp_list),
...
Plots.plot!(
    ...
    P0_100,
    xticks = -5:1:-1,
    label = "L:100, Unpaused Analytical",
    lw = 6,
)
...
Plots.plot!(
    conc_ana,
    T0_ana1350 ./ (T0_ana1350 .+ Tp_ana1350),
    lw = 6,
    label = "L1350, new analytical",
)
...
Plots.plot(
    ylabel = "Average pausing position",
    xlabel = "log(kp)",
    legend = :topright,
    ylims = (0, 1),
    yticks = 0:0.5:1,
...
Plots.plot!(
    log10.(kp_list),
    T0_100 ./ (T0_100 .+ Tp_100),
    xticks = -5:1:-1,
    label = "L:100, Analytical",
    lw = 6,
)
Plots.plot!(
    log10.(kp_list),
...
dp1350_ins =
    calc_density_jam_only_instant_fill.(km, kp_list, l_ribosome, 1350, init, elong, 120)
...
Plots.plot!(
    conc_test[18:29],
    dtot_1350_2[18:29],
    label = "L:1350, paused Analytical (Instant Fill)",
    lw = 6,
)
...
run_Gillespie_for_concentration(
    outputs,
    x,
    Conc,
    k₋,
...
conc
results0 = run_Gillespie_for_concentration(
    outputs,
    1000,
    conc,
    k₋,
...
results60 = run_Gillespie_for_concentration(
    outputs,
    1000,
    conc,
    k₋,
...
results120 = run_Gillespie_for_concentration(
    outputs,
    1000,
    conc,
    k₋,
...
results300 = run_Gillespie_for_concentration(
    outputs,
    1000,
    conc,
    k₋,
...
results2 = run_Gillespie_for_deg_list(
    outputs,
    5000,
    2,
    k₋,
...
results6 = run_Gillespie_for_deg_list(
    outputs,
    500,
    6,
    k₋,
...
results10 = run_Gillespie_for_deg_list(
    outputs,
    500,
    10,
    k₋,
...
plot(
    conc,
    savitzky_golay(prot_syn60 ./ prot_syn0, 5, 2).y,
    label = "Fold change 60",
    lw = 6,
)
plot!(
    conc,
    savitzky_golay(prot_syn120 ./ prot_syn0, 5, 2).y,
    label = "Fold change 120",
    lw = 6,
)
plot!(
    conc,
    savitzky_golay(prot_syn300 ./ prot_syn0, 5, 2).y,
    label = "Fold change 300",
    lw = 6,
)
...
function run_Gillespie_for_init_list(
    outputs,
    x,
    α_list,
    k₋,
    ...
    data = save_data_to_dict(outputs, saving_dict, results, α_list)

    return data
end

function run_Gillespie_for_concentration(
    outputs,
    x,
    Conc,
    k₋,
    ...
    data = save_data_to_dict(outputs, saving_dict, results, k₊_list)
...
function run_Gillespie_for_deg_list(
    outputs,
    x,
    Conc,
    k₋,
    ...
    data = save_data_to_dict(outputs, saving_dict, results, deg_list)
...
for L in L_list0
    t_data = run_Gillespie_for_init_list(
        outputs,
        10000,
        α_list,
        k₋,
...
tmp_tot =
    vcat(transcription_data0, transcription_data1, transcription_data2, transcription_data3)
