...
begin
    ...
    α_crit_hat_fct(k₋, k₊, ϵ) =
        ρ_max_fct(k₋, k₊, ϵ) * ϵ * fa_hat(k₋, k₊, ρ_max_fct(k₋, k₊, ϵ), ϵ)
    α_crit_hat_ext(k₋, k₊, ϵ, ρ_num) =
        ρ_num * ϵ * fa_hat(k₋, k₊, ρ_num, ϵ) / (l_ribosome - ρ_num*l_ribosome + ρ_num)

    ρ_LD_Wang_ext(init, elong, l_ribosome, kp, km) =
        init * l_ribosome /
        (elong*fa_hat_ext(km, kp, ρ, ϵ, l_ribosome) + init*(l_ribosome+1))

    ρ_max_fct(k₋, k₊, γ) =
        -(k₋+k₊)/(γ*fp(k₋, k₊)) +
        sqrt(((k₋+k₊) / (γ*fp(k₋, k₊)))^2 + (k₋+k₊)/(γ*fp(k₋, k₊)))

    Wang(ρ, k₋, k₊, γ) = γ * ρ * (1-ρ) * k₋ / (k₋ + k₊ + γ * ρ * fp(k₋, k₊)) #* Φ1(k₋, k₊)

    # oben boundary functions wang
    Wang_HD(β, k₋, k₊, γ) =
        γ * ρ_HD(β, γ) * (1-ρ_HD(β, γ)) * k₋ / (k₋ + k₊ + γ * ρ_HD(β, γ) * fp(k₋, k₊))
    Wang_HD_new(β, k₋, k₊, ϵ) =
        ϵ * ρ_HD_new(β, ϵ, k₋, k₊) * (1-ρ_HD_new(β, ϵ, k₋, k₊)) * k₋ /
        (k₋ + k₊ + ϵ * ρ_HD_new(β, ϵ, k₋, k₊) * fp(k₋, k₊))

    Wang_LD(α, k₋, k₊, γ) =
        γ * ρ_LD(α, k₋, k₊, γ) * (1 - ρ_LD(α, k₋, k₊, γ)) * k₋ /
        (k₋ + k₊ + γ * ρ_LD(α, k₋, k₊, γ) * fp(k₋, k₊))

    ρ_HD(β, γ) = (1 - β/γ)
    ρ_LD(α, k₋, k₊, ϵ) = α * (k₊+k₋) / (ϵ * (k₋ - α*fp(k₋, k₊)))
    ρ_HD_new(β, ϵ, k₋, k₊) =
        (ϵ * k₋ - fa(k₋, k₊) * β * (k₋ + k₊)) / (β * ϵ * fp(k₋, k₊) * fa(k₋, k₊)+ϵ * k₋)
    ...
    J_ext_k₊(γ, l_ribosome, k₋, k₊, ρ) =
        k₋*γ*ρ*(1-ρ*l_ribosome) / ((1-ρ*(l_ribosome-1)) * (k₋ + k₊ + γ*ρ*Φ(k₋, k₊)))
    ...
    J_whoosh(γ, ρ, L, k₋, k₊) =
        γ * ρ * (1-ρ) * P0(k₋, k₊, ρ, L) + Wang(ρ, k₋, k₊, γ) * (1-P0(k₋, k₊, ρ, L))
    Woosh_fraction(γ, ρ, L, k₋, k₊) =
        γ * ρ * (1-ρ) * pw_fct(ρ, L, k₋, k₊) / J_whoosh(γ, ρ, L, k₋, k₊)
    Cluster_Wang_fraction(γ, ρ, L, k₋, k₊) =
        Wang(ρ, k₋, k₊, γ) * (1-pw_fct(ρ, L, k₋, k₊)) / J_whoosh(γ, ρ, L, k₋, k₊)
    ...
    J_pTASEP_LD_standard_finitesize(α, k₋, k₊, ϵ) =
        (1-P0(k₋, k₊, α/ϵ, L)) * Wang_LD(α, k₋, k₊, ϵ)
    J_pTASEP_MC_standard_finitesize(k₋, k₊, ρ, ϵ) =
        (1-P0(k₋, k₊, 0.5, L)) * Wang(ρ, k₋, k₊, ϵ)
...
function av_current_half(ρ, L, γ, t)
    ...
    if t>=ρ*L
        ...
        if t>=L/(4*ρ)
            return (
                ρ^2*L/6+(ρ/3)*(ρ*L+3*L/(4*ρ)-2*L)+ρ*(1-ρ)*t-L/4*(1-ρ)-(L^2/48)*(4*ρ/L-1/t)
            )*(γ/t)
        end
    end
...
function find_crit_value_num(α, k₋, k₊_list, γ)
    d
    for k₊ in k₊_list
...
function time_in_paused(init, l_ribosome, elong, L, kp, km)
    ...
    avg_pausing_pos =
        analyitcal_total_expected_pausing_position_extended(init, elong, L, kp, l_ribosome)
    w2 = time_clogged_OBC_non_iter(km, kp, l_ribosome, avg_pausing_pos)
...
function current_OBC(L, kp, km, l_ribosome, init, elong)
    return J0_extended(L, kp, km, l_ribosome, init, elong) +
           Jc_extended(L, kp, km, l_ribosome, init, elong)
end
...
function current_OBC_itr(L, kp, km, l_ribosome, α, ϵ)
    return J0_extended_itr(L, kp, km, l_ribosome, α, ϵ) +
           Jc_extended_itr(L, kp, km, l_ribosome, α, ϵ)
end
...
function instantaneousDensityProfile_ext(
    L::Int,
    l_ribosome::Int,
    alpha::Float64,
    epsilon::Float64,
...
function analytical_expected_value_raiyleigh_position(init, elong, L, kp)
    return 0.5 *
           (elong - init) *
           analytical_expected_value_raiyleigh_time(init, elong, L, kp)
end
...
function analytical_expected_value_raiyleigh_position_extended(
    init,
    elong,
    L,
    kp,
    ...
    return s *
           analytical_expected_value_raiyleigh_time_extended(init, elong, L, kp, l_ribosome)
end

function analyitcal_total_expected_pausing_position_extended(init, elong, L, kp, l_ribosome)
    E1 = analytical_expected_value_raiyleigh_position_extended(
        init,
        elong,
        L,
        kp,
