...
function calculate_avg_pause_position_robust(alpha, epsilon, L, kp, tau; ell = 10)
    ...
    F0_L = (sqrt(pi)/2) * erf(u_L);
    F0_0 = (sqrt(pi)/2) * erf(u_0)
    F1_L = -0.5 * exp(-u_L^2);
    F1_0 = -0.5 * exp(-u_0^2)
    val_F0 = F0_L - F0_0
    val_F1 = F1_L - F1_0
...
function calc_J_unpaused_contribution(init, elong, L, kp, l_ribosome, tau)
    ...
    if v <= 0
        ;
        v = elong;
    end # Safety for high initiation
    tL = L / v
...
function calc_density_paused_total(km, kp, l_ribosome, L, init, elong, tau)
    ...
    # --- 2. Part A: The Jam ---
    rho_jam_contrib =
        calculate_density_jam_only_exact2(km, kp, l_ribosome, L, init, elong, tau)
...
function calculate_state_densities(L, kp, km, l_ribosome, init, elong, tau)
    ...
    if kp <= 1e-20
        ...
        # ---------------------------------------------------------
        avg_pos =
            calculate_avg_pause_position_robust(init, elong, L, kp, tau; ell = l_ribosome)
        N_max = avg_pos / l_ribosome
...
function calculate_state_densities_corrected_plateau(
    L,
    kp,
    km,
    l_ribosome,
...
function calculate_state_densities_corrected_platea2(
    L,
    kp,
    km,
    l_ribosome,
...
function calculate_state_densities_corrected_plateau4(
    L,
    kp,
    km,
    l_ribosome,
    ...
    if kp <= 1e-20 || P_entry <= 1e-20
        ...
        term_linear_int =
            ((1.0 / k_eff^2) * (1.0 - E_g_comb)) - ((t_grow / k_eff) * E_g_comb)
        ...
        # --- FINAL ---
        Total_Load_Conditional =
            load_growth + load_plateau + load_dissolution + Total_Load_Runoff
        rhoP_weighted_contrib = P_entry * (Total_Load_Conditional / (L * tau))
...
"""
function calculate_rho_p_unified(
    L,
    kp,
    km,
    ell,
    ...
    if Xs >= L
        ...
        t_start = (L - Xs) / v;
        t_exit = (L - Xf) / v
        E_start = exp(-t_start/tau);
        E_ro = exp(-(t_exit-t_start)/tau)
...
function calculate_rho_p_conditional(L, kp, km, ell, alpha, epsilon, tau)
    ...
    function Lambda(t)
        return (t < t_L) ? (0.5 * J_ss * kp * t^2) :
               (Lambda_at_tL + rho_ss * L * kp * (t - t_L))
    end
    ...
    function get_L_ro_transient(Xf)
        if 2.0 * Xf >= L
            ;
            return get_L_ro_ss(Xf);
        end
        t_start = (L - 2.0*Xf) / v
        t_ro = Xf / v
        return exp(-t_start / tau) *
               ((rho_ss * Xf * tau) - (J_ss * tau^2 * (1.0 - exp(-t_ro / tau))))
    end
    ...
    integrand_filling(t) =
        F(t) * (get_L_cf(v*t/2.0) + get_L_c(v*t/2.0) + get_L_ro_transient(v*t/2.0))
