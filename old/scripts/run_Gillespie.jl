...
function times_from_dict(dict)
    tw = collect(
        values(
            sort(
                Dict(
                    key => value[1] for (key, value) in sort(dict["Results"]["total_time"])
                ),
            ),
        ),
    )
    tc = collect(
        values(
            sort(
                Dict(
                    key => value[2] for (key, value) in sort(dict["Results"]["total_time"])
                ),
            ),
        ),
    )
    t = collect(
        values(
            sort(
                Dict(
                    key => value[3] for (key, value) in sort(dict["Results"]["total_time"])
                ),
            ),
        ),
    )
...
function densities_from_dict(dict)
    m = collect(
        values(
            sort(
                Dict(key => value[1] for (key, value) in sort(dict["Results"]["density"])),
            ),
        ),
    )
    p = collect(
        values(
            sort(
                Dict(key => value[2] for (key, value) in sort(dict["Results"]["density"])),
            ),
        ),
    )
    j = collect(
        values(
            sort(
                Dict(key => value[3] for (key, value) in sort(dict["Results"]["density"])),
            ),
        ),
    )
    tot = collect(
        values(
            sort(
                Dict(key => value[4] for (key, value) in sort(dict["Results"]["density"])),
            ),
        ),
    )
    tot_unp = collect(
        values(
            sort(
                Dict(key => value[5] for (key, value) in sort(dict["Results"]["density"])),
            ),
        ),
    )
    tot_p = collect(
        values(
            sort(
                Dict(key => value[6] for (key, value) in sort(dict["Results"]["density"])),
            ),
        ),
    )
...
function current_from_dict(dict)
    Jw = collect(
        values(
            sort(
                Dict(key => value[1] for (key, value) in sort(dict["Results"]["current"])),
            ),
        ),
    )
    Jc = collect(
        values(
            sort(
                Dict(key => value[2] for (key, value) in sort(dict["Results"]["current"])),
            ),
        ),
    )
    J = collect(
        values(
            sort(
                Dict(key => value[3] for (key, value) in sort(dict["Results"]["current"])),
            ),
        ),
    )
    return Jw, Jc, J
...
data_LD500_k₋001_k₊0001 = save_data_to_dict(
    outputs,
    simulation_data_LD500_k₋001_k₊0001,
    results_LD500_k₋001_k₊0001,
    α_list,
)
...
data_HD250_k₋01_k₊001 = save_data_to_dict(
    outputs,
    simulation_data_HD250_k₋01_k₊001,
    results_HD250_k₋01_k₊001,
    β_list,
)
...
data_LD_L1000_k₋0001_k₊001 =
    save_data_to_dict(outputs, simulation_data_LD, results_LD_L1000_k₋0001_k₊001, α_list)
...
scatter!(
    k₊_list_axis,
    ρ_max_fct.(10^-3, k₊_list, 1),
    label = "ρ_max pTASEP",
    markersize = markersize,
    markercolor = colors[end],
    markershape = marker_shapes[4],
)
...
scatter!(
    ...
    markershape = marker_shapes[3],
)

plot!(
    k₊_list_axis,
    open_bio_light.(0.1, k₊_list, 10^-3, 50, 1),
    label = false,
    linewidth = linewidth,
    linecolor = colors[1],
)
plot!(
    k₊_list_axis,
    open_bio_light.(0.1, k₊_list, 10^-3, 100, 1),
    label = false,
    linewidth = linewidth,
    linecolor = colors[2],
)
plot!(
    k₊_list_axis,
    open_bio_light.(0.1, k₊_list, 10^-3, 150, 1),
    label = false,
    linewidth = linewidth,
    linecolor = colors[5],
)
...
dens_HD_250 = collect(
    values(
        sort(
            Dict(
                key => value[4] for
                (key, value) in sort(data_HD_L250["Results"]["density"])
            ),
        ),
    ),
)
dens_LD_250 = collect(
    values(
        sort(
            Dict(
                key => value[4] for
                (key, value) in sort(data_LD_L250["Results"]["density"])
            ),
        ),
    ),
)
...
scatter(
    ρ_list,
    J500[index2, index1][1:end, 1],
    label = "PBC Numerics",
    markersize = 5,
    markercolor = colors[15],
)
