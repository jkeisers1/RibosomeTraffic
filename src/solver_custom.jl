...
# 1. For Active ribosomes, we return the pause rate
get_rate(::Active, model::TranscriptModel) = model.k_pause

# 2. For Paused ribosomes, we return the unpause rate
get_rate(::Paused, model::TranscriptModel) = model.k_unpause

# 3. For Empty or Jammed, the switching rate is 0.0
get_rate(::RibosomeState, model::TranscriptModel) = zero(model.k_pause)

@inline get_switch_rate(::EmptySite, model) = zero(model.k_pause)
@inline get_switch_rate(::Active, model) = model.k_pause
@inline get_switch_rate(::Paused, model) = model.k_unpause

# Convert integer state to Type for dispatch
@inline decode_state(s::UInt8) = (s == 0) ? EmptySite() : (s == 1 ? Active() : Paused())

"""
this function updates the local rates AFTER a the system already changed
"""
# --- 1. Rate Updates (Robust Jamming Logic) ---
function update_local_rates!(
    state::SimState{T},
    model::TranscriptModel{T},
    i::Int,
) where {T}
    # --- just double checking the switching takes place on the lattice and nothing messed up ---
    if i < 1 || i > model.L
        ;
        return;
    end
    ...
    # --- Old Rate ---
    # we only need to check if the site was mobile because we have an explicit counter
    was_mobile = (state.rate_elong[i] > zero(T))

    # --- Remove old rates ---
    state.total_rate_elong -= state.rate_elong[i]
    state.total_rate_switch -= state.rate_switch[i]

    # --- STEP 3: Check for "The Void" (Empty Site) ---
    # If the site is empty, we are done all rates become zero!
    if state.lattice[i] == 0
        state.rate_elong[i] = zero(T)
        state.rate_switch[i] = zero(T)

        # If it used to be moving, it's gone now, so decrease count.
        if was_mobile
    ...
    # --- Determine New particle state ---
    # if the site is still occupied the system change must have been a switch
    status_val = state.internal_states[i]
    status_type = decode_state(status_val)

    # A. Calculate Switching
    new_switch_rate = get_switch_rate(status_type, model)
    state.rate_switch[i] = new_switch_rate
    state.total_rate_switch += new_switch_rate

    # B. Calculate Elongation
    current_elong_rate = zero(T)

    # Only Active ribosomes (status 1) can move
    # this means the switching happened from paused to active and the active particle is not jammed by a paused one
    if status_val == 1
        if i == model.L # particle on last site unpaused
            current_elong_rate = model.β
        else
            # Check neighbor (fast direct access)
            check_index = i + model.l_ribosome
            # Is blocked if: Neighbor exists AND is occupied
            is_blocked = (check_index <= model.L && state.lattice[check_index] > 0)
    ...
    # --- Update mobile ---
    # Did we change from mobile to stuck (or vice versa)?
    is_mobile = (current_elong_rate > zero(T))
...
function update_initiation_rate!(state::SimState{T}, model::TranscriptModel{T}) where {T}
    # MEMORY FIX: Use a view if you use high-level functions, or manual loop
    # Ideally: any(@view(state.lattice[1:model.l_ribosome]) .== 1)
    # Your manual loop was actually efficient! But let's verify no hidden allocs.

    limit = min(model.L, model.l_ribosome)
    # Using a view is safer for readability without allocation
    is_blocked = any(x -> x == 1, @view state.lattice[1:limit])
